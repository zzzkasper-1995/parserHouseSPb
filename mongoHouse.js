...
  }
	...
	fs.readdir(`./html`, async (err, dirs) => {
		dirs.forEach(dir => {
			if(dir.indexOf('.') === -1) {
				fs.readdir(`./html/${dir}`, async (err, files) => {
					//проверяем наличие файла со списком домов
					if (!fs.existsSync(`./html/${dir}/main.json`)) {
						logger.error(`./html/${dir}/main.json not exist!`);
						return 0;
					}

					const dataText = fs.readFileSync(`./html/${dir}/main.json`, 'utf-8');
					...
					for (let element of houses) {
						...
						const jsonFileName = `./html/${dir}/${element.url.split('/')[3]}.json`;
